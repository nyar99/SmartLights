...
#define SSID_NAME "ATTiwmRXCT"
#define SEC_TYPE ESP8266_ENCRYPT_MODE_WPA2_PSK
#define PASSKEY "u55ek8yiiyw="
...
//-------------------bufferRotate -------------------
// - right shifts a buffer of fixed size
// Inputs: none
// Outputs: none
void bufferRotate(char letter){
  for (int i = 0; i < BUFFERSIZE-1; i++) {
      serial_buffer[i] = serial_buffer[i+1];
  }
  serial_buffer[BUFFERSIZE-1] = letter;
}
//-------------------printTempo -------------------
// - prints string to uart1 if it passes tempo conditions
// Inputs: none
// Outputs: none
...
//         echo to UART0?
// Outputs: none
void Arduino_InitUART(uint32_t baud, int echo){ volatile int delay;
  ESP8266_EchoResponse = echo;
...
void ESP8266_DisableRXInterrupt(void){
  NVIC_DIS1_R = 0x01<<(61-32);           // enable interrupt 61 in NVIC
}
...
void DelayMsSearching(uint32_t n){
	//UART_OutString("Delay searching...\r\n");
  volatile uint32_t time;
  while(n){
		//UART_OutUDec(n);
...
=======================================================================
*/
void ESP8266_BasicInit(uint32_t baud){
...
int ESP8266_Reset(){int try=MAXTRY;
  SearchStart("ready");
  while(try){
    GPIO_PORTE_DATA_R &= ~0x02; // reset low
    DelayMs(10);
    GPIO_PORTE_DATA_R |= 0x02; // reset high
...
int ESP8266_JoinAccessPoint(const char* ssid, const char* password){
  int try=MAXTRY;
	UART_OutString("Joining Access Point...\r\n");
  SearchStart("ok");
  while(try){
